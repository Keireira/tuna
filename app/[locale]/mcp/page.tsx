import McpPage from '@/views/mcp-page/mcp-page';
...
export const generateMetadata = async ({ params }: TProps): Promise<Metadata> => {
	...
	return getPageMetadata(locale, 'mcp');
};
...
const Page = async ({ params }: TProps) => {
	...
	return (
		<>
			<StructuredDataScript id="uha-mcp-jsonld" json={await getPageStructuredData(locale, 'mcp')} />
			<McpPage locale={locale} />
		</>
	);
};
